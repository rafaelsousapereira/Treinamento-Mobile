package com.treinamentomobile.projetoudf.viewmodel

import androidx.compose.runtime.State
import androidx.compose.runtime.mutableStateOf
import androidx.lifecycle.ViewModel
import androidx.lifecycle.viewModelScope
import com.treinamentomobile.projetoudf.model.Person
...
class PersonViewModel : ViewModel() {
    ...
    sealed class PersonAction {
        ...
        data class RemoverPerson(val person: Person) : PersonAction()
        data object AddPerson : PersonAction()
    }

    // Função para administrar as açoes
    fun onAction(action: PersonAction) {
        when (action) {
            ...
            is PersonAction.AddPerson -> {
                ...
                // Side Effects
                viewModelScope.launch { _uiEffect.emit("Pessoa cadastrada com sucesso!") }
            }

            // Remover pessoa
            is PersonAction.RemoverPerson -> {
                _state.value = _state.value.copy(
                    persons = _state.value.persons - action.person
                )

                // Side Effects
                viewModelScope.launch { _uiEffect.emit("Pessoa removida com sucesso!") }
            }
        }
    }
...
/**
 * Classe selada
 * Object -> Ao trabalhar com objetos completos
